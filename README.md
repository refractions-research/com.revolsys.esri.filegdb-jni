...
3. Log into the docker console as root and setup a number of tools on the docker instance:
docker ps
...
5. Setup Jenkins via the web interface using the defaults

6. On Jenkins (via the web interface) installed the GitHub Integration Plugin
Settings -> Plugins 
Added: "GitHub Integration Plugin"

7. On Jenkins (via the web interface) configured a number of Tools
Settings -> Tools
Git Installations -> Ensure this was setup correctly 

8. On local windows machine, download and install the community edition of Visual Studio. In this case I used version 2022 to match the ESRI build version.

9. In Jenkins create a windows-agent node (Settings -> Nodes), 
Name: windows_agent
...
10. Updated the versions of all the files in the project to point to the versions being used:
...
11. Create a new Jenkins build item. 
Type: Pipeline
Name: Build
...
12. Run the newly created build item.
...
13. Copy the build files off the docker instance and put them into our github maven repo:
